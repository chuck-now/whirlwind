...
public static class Extensions
{
    ...
    public static string GetFileName(this string url)
    {
        ...
        return array[^1].Split(new char[1]
        {
            '?'
        })[0];
    }
    ...
    public static SqlDbType SqlTypeNameToSqlDbType(this string @this)
    {
        return (@this.ToLower()) switch
        {
            ...
            "xml" => SqlDbType.Xml,
            _ => throw new Exception($"Unsupported Type: {@this}"),
        };
    }
    ...
    public static ExpandoObject ToExpando(this IDictionary<string, object> dictionary)
    {
        ...
        {
            if (item2.Value is IDictionary<string, object> dictionary1)
            {
                ExpandoObject value = dictionary1.ToExpando();
                dictionary2.Add(item2.Key, value);
            }
            else if (item2.Value is ICollection collection)
            {
                List<object> list = new List<object>();
                foreach (object item3 in collection)
                {
                    if (item3 is IDictionary<string, object> dictionary3)
                    {
                        ExpandoObject item = dictionary3.ToExpando();
                        list.Add(item);
                    }
                    else
                    {
    ...
    /// <param name="asBaseLibMinus"></param>
    /// <param name="compareTo"></param>
    /// <param name="includeSign"></param>
    /// <returns></returns>
    public static string ToRelativeTime(this DateTime dt, bool includeTime = true, bool asBaseLibMinus = false, DateTime? compareTo = null, bool includeSign = true)
    {
