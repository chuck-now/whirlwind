...
namespace BaseLib.EntityFramework
{
    ...
    public abstract class DbContextTypeMatcher<TBaseDbContext> : IDbContextTypeMatcher, ISingletonDependency
    {
        ...
        public virtual Type GetConcreteType(Type sourceDbContextType)
        {
            ...
            {
                throw new BaseLibException("Could not find a concrete implementation of given DbContext type: " + sourceDbContextType.AssemblyQualifiedName);
            }
        ...
        private void CheckCurrentUow()
        {
            ...
            {
                throw new BaseLibException("GetConcreteType method should be called in a UOW.");
            }
        }
        ...
        private static Type GetDefaultDbContextType(List<Type> dbContextTypes, Type sourceDbContextType)
        {
            ...
            throw new BaseLibException($"Found more than one concrete type for given DbContext Type ({sourceDbContextType}). Found types: {(from c in dbContextTypes select c.AssemblyQualifiedName).JoinAsString(", ")}.");
        }
