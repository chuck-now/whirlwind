...
using BaseLib.Event.Bus.Factories;
using BaseLib.Event.Bus.Factories.Internals;
using BaseLib.Event.Bus.Handlers;
using BaseLib.Event.Bus.Handlers.Internals;
using System;
...
namespace BaseLib.Event.Bus
{
    ...
    public class EventBus : IEventBus
    {
        ...
        {
            GetOrCreateHandlerFactories(eventType).Locking(delegate (List<IEventHandlerFactory> factories)
            {
        ...
        {
            GetOrCreateHandlerFactories(typeof(TEventData)).Locking(delegate (List<IEventHandlerFactory> factories)
            {
                factories.RemoveAll(delegate (IEventHandlerFactory factory)
                {
                    if (!(factory is SingleInstanceHandlerFactory singleInstanceHandlerFactory))
                    {
                        return false;
                    }
                    if (!(singleInstanceHandlerFactory.HandlerInstance is ActionEventHandler<TEventData> actionEventHandler))
                    {
                        return false;
                    }
                    return actionEventHandler.Action == action;
                });
            });
        }
        ...
        public void Unregister(Type eventType, IEventHandler handler)
        {
            GetOrCreateHandlerFactories(eventType).Locking(delegate (List<IEventHandlerFactory> factories)
            {
                factories.RemoveAll((IEventHandlerFactory factory) => factory is SingleInstanceHandlerFactory && (factory as SingleInstanceHandlerFactory).HandlerInstance == handler);
            });
        }
        ...
        {
            GetOrCreateHandlerFactories(eventType).Locking((List<IEventHandlerFactory> factories) => factories.Remove(factory));
        }
        ...
        {
            List<EventTypeWithEventHandlerFactories> list = new List<EventTypeWithEventHandlerFactories>();
